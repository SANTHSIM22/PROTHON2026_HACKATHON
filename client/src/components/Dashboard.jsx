...
const StructuredDataPanel = ({ data }) => {
  ...
  const totalValues = (rawJson.match(/:/g) || []).length;
  ...
  const tokenize = (line) => {
    ...
    while ((match = regex.exec(line)) !== null) {
      if (match.index > lastIndex) tokens.push({ type: 'plain', value: line.slice(lastIndex, match.index) });
      if (match[1])      tokens.push({ type: 'key',     value: match[1] + line.slice(match.index + match[1].length, regex.lastIndex) });
      ...
      else               tokens.push({ type: 'punct',   value: match[6] });
      lastIndex = regex.lastIndex;
    }
    if (lastIndex < line.length) tokens.push({ type: 'plain', value: line.slice(lastIndex) });
    return tokens;
  };
  ...
  const stats = [
    { label: 'top-level keys', value: topLevelKeys.length },
    { label: 'total fields',   value: totalValues },
    { label: 'lines',          value: lines.length },
    { label: 'bytes',          value: rawJson.length },
  ];
  ...
  return (
    ...
    >
      {/* ── Title bar ── */}
      <div
        className="flex items-center justify-between px-5 py-3.5 border-b border-[#1a3329]"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="w-3 h-3 rounded-full" style={{ background: '#ffbd2e', opacity: 0.7 }} />
            <div className="w-3 h-3 rounded-full" style={{ background: '#28c840', opacity: 0.7 }} />
          </div>
          <div className="w-px h-4 bg-[#1a3329]" />
          {/* Icon + filename */}
          <div className="flex items-center gap-2">
            <svg className="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2">
            ...
            <span
              ...
              style={{ background: '#D97706' + '20', color: '#D97706', fontFamily: 'var(--font-mono)' }}
            >
              json
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.color = '#a7c5b8'; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }}
            onMouseLeave={e => { e.currentTarget.style.color = '#4d7a6a'; e.currentTarget.style.background = 'transparent'; }}
          >
            <svg
          ...
          <button
            ...
            style={{
              fontFamily: 'var(--font-mono)',
              background: copied ? 'rgba(21, 128, 61, 0.15)' : 'rgba(255,255,255,0.04)',
              border: `1px solid ${copied ? 'rgba(21,128,61,0.3)' : 'rgba(255,255,255,0.07)'}`,
              color: copied ? '#4ade80' : '#4d7a6a',
            }}
          >
            {copied ? (
              <>
      ...
      {/* ── Stats strip ── */}
      <div
        className="flex items-stretch border-b border-[#1a3329]"
        style={{ background: '#0b1915' }}
      >
        {stats.map((stat, i) => (
          <div
            key={i}
            className="flex items-center gap-2 px-5 py-2.5 border-r border-[#1a3329] last:border-r-0"
          >
            <span className="text-[13px] font-bold tabular-nums" style={{ color: '#D97706', fontFamily: 'var(--font-mono)' }}>
              {stat.value}
            </span>
            <span className="text-[10px] uppercase tracking-widest" style={{ color: '#2d4a3e', fontFamily: 'var(--font-mono)' }}>
              {stat.label}
            </span>
          </div>
        ))}

        {/* Key pills — right side */}
        <div className="ml-auto flex items-center gap-1.5 px-5 py-2.5">
          {topLevelKeys.slice(0, 5).map((k, i) => (
            <span
              key={i}
          ...
          {topLevelKeys.length > 5 && (
            <span className="text-[10px]" style={{ color: '#2d4a3e', fontFamily: 'var(--font-mono)' }}>
              +{topLevelKeys.length - 5}
            </span>
          )}
        </div>
      ...
      {!collapsed ? (
        <div
          ...
          style={{
            ...
            padding: '16px 20px',
            scrollbarWidth: 'thin',
            scrollbarColor: '#1a3329 transparent',
          }}
        >
            ...
            {lines.map((line, lineIdx) => {
              ...
              return (
                <div
                  ...
                  style={{ borderRadius: '4px', margin: '0 -4px', padding: '0 4px' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                >
                  {/* Line number */}
                  <span
                    className="select-none text-right flex-shrink-0 mr-5"
                    style={{
                      fontFamily: 'var(--font-mono)',
                      fontSize: '11px',
                      lineHeight: '1.75',
                      color: '#1e3a2e',
                      width: `${String(lines.length).length * 8 + 4}px`,
                      paddingTop: '1px',
                    }}
                  >
                    {lineIdx + 1}
                  ...
                  >
                    {tokens.map((tok, ti) => (
                      <span key={ti} style={{ color: colorMap[tok.type] || colorMap.plain }}>
                        {tok.value}
                      </span>
                    ))}
                  </span>
        ...
        <div className="flex items-center justify-center gap-3 py-5">
          <svg className="w-4 h-4" style={{ color: '#2d4a3e' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M9 18V5l12-2v13"/>
            <circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>
          </svg>
          <span className="text-[12px]" style={{ color: '#2d4a3e', fontFamily: 'var(--font-mono)' }}>
            {lines.length} lines · {rawJson.length} bytes
          </span>
          <button
            onClick={() => setCollapsed(false)}
            className="text-[12px] underline underline-offset-2 transition-colors duration-200"
            style={{ color: '#4d7a6a', fontFamily: 'var(--font-mono)' }}
            onMouseEnter={e => { e.currentTarget.style.color = '#a7c5b8'; }}
            onMouseLeave={e => { e.currentTarget.style.color = '#4d7a6a'; }}
          >
            show
          </button>
        </div>
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        {processedData && (
          <div className="space-y-6">
            ...
            {/* ── Structured Data Panel (improved) ── */}
            {processedData.structuredData && (
              <StructuredDataPanel data={processedData.structuredData} />
            )}
